...
void init()
{
    ...
    {
        MessageBoxA(g_hWnd, "创建命名管道失败！", "f", MB_OK); 
        hPipe = NULL;
        return;
    }
    HANDLE hEvent;
    ...
    if (!hEvent)
    {
        MessageBoxA(g_hWnd, "创建事件对象失败！", "f", MB_OK);
        CloseHandle(hPipe);
        hPipe = NULL;
        return;
    }
    ...
    if (!ConnectNamedPipe(hPipe, &ovlap))
    {
        ...
        {
            MessageBoxA(g_hWnd, "等待客户端连接失败！", "f", MB_OK);
            CloseHandle(hPipe);
            CloseHandle(hEvent);
            hPipe = NULL;
            return;
    ...
    {
        MessageBoxA(g_hWnd, "等待对象失败！", "f", MB_OK);
        CloseHandle(hPipe);
        CloseHandle(hEvent);
        hPipe = NULL;
        return;
...
void OnPipeRead()
{
    ...
    {
        MessageBoxA(g_hWnd, "读取数据失败！", "f", MB_OK);
        return;
    }
    MessageBoxA(g_hWnd, buf, "f", MB_OK); 
}
...
void OnPipeWrite()
{
    ...
    {
        MessageBoxA(g_hWnd, "写入数据失败！", "f", MB_OK);
        return;
    }
    ...
    case WM_COMMAND:
        {
            ...
            case IDM_ABOUT:
                ...
                break;
            case ID_COMMUNICATE_INIT:
                init();
                break;
            case ID_COMMUNICATE_SEND:
                OnPipeWrite();
                break;
            case ID_COMMUNICATE_RECEIVE:
                OnPipeRead();
                break;
            case IDM_EXIT:
                DestroyWindow(hWnd);
